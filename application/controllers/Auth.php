...
class Auth extends CI_Controller
{
    ...
    public function registration()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert"> Congratation! your account has been registered. Please activate your account</div>');
            redirect('auth');
        }
    }
    ...
    private function _sendEmail($token, $type)
    {
        $config = [
            ...
            'smtp_user' => 'idhamkholid965@gmail.com',
            'smtp_pass' => 'lunar1234',
            'smtp_port' => 465,
            'mailtype' => 'html',
            'charset' => 'utf-8',
        ...
        $this->email->from('idhamkholid965@gmail.com', 'Idham Kholid');
        $this->email->to($this->input->post('email'));
        if ($type == 'verify') {
            $this->email->subject('Account Verification');
            $this->email->message(
                'Click this link to verify your account : <a href="' . base_url('auth/verify?email=') . $this->input->post('email') . '&token=' . urlencode($token)
                    . '">Activate</a>'
            );
        } else if ($type == 'forgot') {
            $this->email->subject('Reset Password');
            $this->email->message(
                'Click this link to reset your password : <a href="' . base_url('auth/resetpassword?email=') . $this->input->post('email') . '&token=' . urlencode($token)
                    . '">Reset Password</a>'
            );
        }
    ...
    public function changePassword()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert"> You password has been changed</div>');
            redirect('auth');
        }
    }
